...
def fake_image(img):
    ...
    '''
    result=img.flatten()
    samples=np.random.choice(len(result),int(img.shape[0]*0.9)*int(img.shape[1]*0.9),replace=True)
    for i in samples:
        ran=(np.random.random_integers(2)-1)*256
        result[i]=ran
    result=np.reshape(result,img.shape)
    return result

def pad_to_window(img,shape1,shape2,value=0):
    padded = value * np.ones(shape=img.shape, dtype=img.dtype)
